...
class Juego {
    ...
    getClick() {
        let i = parseInt(this.getAttribute('id').substring(5)); 
      
// index para buscar el imagen en array
// this la carta que tocamos
        if (cartas_pares.length < 4 && !cartas_abiertas.includes(this.id)) { 
          
//vertif si id de la carta ya esta abierta
document.getElementById(`caja_${i}`).style.background = vector[i];
            if (cartas_pares.length === 0) guardar(this); 
              
// this es la carta que tocamos 
            else if (cartas_pares.length === 2 && cartas_pares[1] !== this.id) {
                guardar(this);
                if (cartas_pares[0] === cartas_pares[2]) {
                    cartas_abiertas.push(cartas_pares[1], cartas_pares[3]); 
                
// guardamos id de todas las cartas abiertas iguales
                    limpiar();
                    if (cartas_abiertas.length === 24) {
                        ...
                        document.getElementById("autor").innerHTML = "Desarrollador: icaito-ricode 05/2022"
